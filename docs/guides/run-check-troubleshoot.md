...
| Picture sideways | The .265 records no rotation, so the picture is shown as its pixels were stored, and they were stored turned | Encode from the master again rather than copying the stream out of its container, then confirm the width and height with `ffprobe` — see [prepare-video.md](prepare-video.md) |
