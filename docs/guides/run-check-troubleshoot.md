...
|---|---|---|
| Picture at the wrong resolution, often 1024x768, after a power cut | The display or projector was still asleep when the Raspberry Pi booted, so the system picked a fallback size and kept it | Set `display_mode` and `kms_force` in the exhibit config, run `sudo dex-exhibit-apply`, reboot — see [configure-exhibit.md](configure-exhibit.md). Where the display lists no whole-number refresh, no mode can be written for it: use `display_mode: auto` and give the connector a forced mode with the trailing `D` alone |
