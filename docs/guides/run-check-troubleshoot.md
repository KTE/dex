...
`n/a` means dexd has not received a value yet; `off` means the player never asked mpv for that counter.
...
|---|---|---|
| Picture at the wrong resolution, often 1024x768, after a power cut | The display or projector was still asleep when the Raspberry Pi booted, so the system picked a fallback size and kept it | Set `display_mode` and `kms_force` in the exhibit config, run `sudo dex-exhibit-apply`, reboot — see [configure-exhibit.md](configure-exhibit.md) |
...
| Restart loop with `Could not find mode matching 3840x2160@60` from mpv | The size exists on this display but that refresh rate does not | Set a refresh rate the connector lists, checked against the `modetest` output below |
| Plays visibly too slow or too fast, with no errors and every counter clean | The sidecar states the wrong frame rate | Write the sidecar again with the correct rate — `dex-sidecar write artwork.265 --fps 30 --force` — then copy the video and its `.json` across together; see [prepare-video.md](prepare-video.md) |
| Picture sideways | ffmpeg dropped the rotation flag when the video came out of its video container | Re-encode with `-noautorotate` — see [prepare-video.md](prepare-video.md) |
