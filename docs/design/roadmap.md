...
- Whether `display_mode: auto` should refuse rather than warn when the connector cannot offer the resolution the sidecar states (see [sidecar.md](sidecar.md)), and whether the example in the refusal message should suggest `auto` at all, now that the package installs no config. A deployed card has since argued both ways: `auto` was the only working value there, because that panel offers one fractional timing and no integer mode matches it, and `auto` is equally what let a mismatched 3840×2160 video reach the player and restart it (see [measurements.md](measurements.md#mode-behaviour-by-sink)).
