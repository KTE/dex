...
- Whether `display_mode: auto` should refuse rather than warn when the connector cannot offer the resolution the sidecar states (see [sidecar.md](sidecar.md)), and whether the example in the refusal message should suggest `auto` at all, now that the package installs no config.
