...
The refresh is an integer, and dexd refuses both fractional forms when it parses the config. The kernel keys each display mode on an integer refresh and mpv matches `--drm-mode` against it (documented; see [vrefresh](../glossary.md#vrefresh)), so a decimal such as `3840x2160@29.97` names the same 30 Hz mode that `@30` names. A rational such as `3840x2160@30000/1001` does not reach mode matching at all: mpv's option parser rejects it (`error setting option (-7)`, observed on mpv 0.40) and the player restarts instead of showing a picture. An integer refresh the connector does not offer fails later, at video-output init (see [startup-checks.md](startup-checks.md)).

A connector whose only timing is fractional therefore has no working integer `display_mode` at all. On one deployed player a 2560×1440 monitor lists that size at 59.95 Hz alone: `2560x1440@60` matches no mode and restarts the player, and dexd refuses the fractional forms before mpv sees them. `auto` is the only value that resolves such a connector, because mpv then takes the connector's preferred mode instead of matching a number against the list (see [measurements.md](measurements.md#mode-behaviour-by-sink)).
