...
//! see `dex_loop::health`'s module doc). F1 acts on that silence in-process,
...
//! `WatchdogSec=` in the unit (`deploy/dex-loop.service`) plus this module.
//!
...
//! NEVER refills (see `dex_loop::health`'s "why the budget never resets"),
...
//! `WatchdogSec=180` (`deploy/dex-loop.service`), systemd needs ~17
...
mod tests {
    ...
    fn socket_addr_resolves_a_path_target() {
        let target = NotifySocketAddr::Path("/tmp/dex-loop-watchdog-example.sock".to_string());
        let addr = socket_addr(&target).expect("path address resolves");
        assert_eq!(
            addr.as_pathname(),
            Some(std::path::Path::new("/tmp/dex-loop-watchdog-example.sock"))
        );
    }
    ...
    fn socket_addr_resolves_an_abstract_target_on_linux() {
        let target = NotifySocketAddr::Abstract("dex-loop-watchdog-test".to_string());
