...
`pos-age=`) at boot and every 10 minutes. The drop counters (`frame-drops=`,
`vo-delayed=`) are cumulative since process start and read `n/a` until the first value
arrives from mpv, or `off` if the subscription failed at startup — never a direct
property read, so the heartbeat itself can never block on a wedged mpv core.
